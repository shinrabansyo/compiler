...
use super::{ADDR_REG, lirgen_expr};

pub fn lirgen_struct_field_init<'src>(ctx: &mut GenContext<'src>, struct_field_init: StructFieldInit<'src>) -> LirBlock {
    ...
    let lir_write = match struct_field_init.size {
        1 => lir!(Sb(offset) ADDR_REG, reg_expr),
        2 => lir!(Sh(offset) ADDR_REG, reg_expr),
        4 => lir!(Sw(offset) ADDR_REG, reg_expr),
        _ => unreachable!(),
    };
