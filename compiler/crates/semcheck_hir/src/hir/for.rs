...
impl<'src> SemCheck<InDep<'src>, ast::For<'src>> for For<'src> {
    async fn check0(mut ctx: InDep<'src>, r#fot: ast::For<'src>) -> anyhow::Result<Self>
    where
        Self: Sized,
    {
        // 初期化文, 増分式, ブロックの意味解析
        let init = VarDecl::check(&mut ctx, r#fot.init).await?;
        let incr = Expr::check(&mut ctx, r#fot.incr).await?;
        let block = Block::check(ctx.clone(), r#fot.block).await?;

        // 条件式の意味解析 & 型チェック
        let cond = Expr::check(&mut ctx, r#fot.cond).await?;
