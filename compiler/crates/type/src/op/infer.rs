...
pub fn ty_infer(a: &Type) -> anyhow::Result<Type> {
    match a {
        // プリミティブ型
        Primitive(Void)     => Ok(Primitive(Void)),
        Primitive(I8)       => Ok(Primitive(I8)),
        Primitive(I16)      => Ok(Primitive(I16)),
        Primitive(I32)      => Ok(Primitive(I32)),
        Primitive(NumConst) => Ok(Primitive(I32)),
    }
}
